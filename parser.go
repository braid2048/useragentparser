...
type userAgentParser struct {
	...
	ModelMapping  map[string]DeviceBrand
	FuzzyMapping  map[string]DeviceBrand
}
...
func (parser *userAgentParser) preDetect(agentString string) (userAgent *UserAgent) {
	...
	if strings.Contains(agentString, "iPhone") {
		return &UserAgent{
			Device: &Device{Brand: DeviceBrandApple, Model: ""},
			Os:     &Os{},
		}
	}
	...
	if strings.Contains(agentString, "Windows NT") {
		return &UserAgent{
			Device: &Device{Brand: DeviceBrandOther, Model: ""},
			Os:     &Os{},
		}
	}
